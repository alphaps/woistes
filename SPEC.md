...
- **EF Core data layer** (`Woistes.Infrastructure`): DbContext, entity configurations (with indexes on Name/FullPath/ParentId), `ICatalogueRepository` + SQL Server implementation with search (LIKE), tree browsing, and DI extension method
  ...
  - **Catalogue deletion uses leaf-first batch removal**: because `Entries` has a self-referencing FK (`ParentId → Id`) with `DeleteBehavior.NoAction`, a single `DELETE WHERE DiskId IN (...)` deadlocks on FK validation for large trees. The fix: a loop deletes entries in batches of 5,000, each batch targeting only leaf nodes (entries with no children), until none remain. Then disks and the catalogue are deleted. This avoids FK conflicts and keeps each batch fast.
  - **Large-import tuning (SqlBulkCopy)**: catalogues like `mypassport1000.CTF` (306k entries) and `120 Go.CTF` (106k entries) use `SqlBulkCopy` on SQL Server for fast bulk insertion, bypassing EF Core's change tracker entirely. The approach: (1) a SQL Server **sequence** (`EntryIdSequence`, increment 1) replaces identity columns on `Entries.Id`; (2) after parsing, the entry tree is flattened and all IDs + ParentIds are assigned client-side via `sp_sequence_get_range`; (3) a custom `IDataReader` (`EntryDataReader`) streams rows to `SqlBulkCopy` (batch size 10,000). This reduces import time from tens of seconds to a few seconds. For non-SQL-Server providers (SQLite in tests, InMemory), the EF fallback path is used (per-disk `AddRange` + `SaveChanges` with `AutoDetectChangesEnabled = false`). Additional tuning still in place: `MaxBatchSize(1000)` for the fallback path, and `Logging` config suppressing EF SQL logging. (Migration gotcha: SQL Server cannot `ALTER COLUMN` to remove IDENTITY — the migration uses raw SQL to drop/recreate the column while preserving data, FKs, and indexes.)
- **Target framework upgrade**: all projects migrated from net8.0 to net10.0
- **ASP.NET Core Web API** (`Woistes.Api`): minimal API with endpoints for CTF upload/import, catalogue CRUD, tree browsing (lazy-load children by disk/parent), and paginated search with glob patterns. **38 integration tests** using WebApplicationFactory + InMemory DB (incl. auth, logout/denied, deep-tree browse regression, import progress, EntryDataReader unit tests).

- **Import progress reporting**: `AddAsync` accepts an optional `IProgress<ImportProgress>` and reports `(DisksSaved, DisksTotal, EntriesSaved, EntriesTotal)` after each disk's bulk insert completes. The Blazor Upload page shows a live progress bar. Reporting is opt-in so the REST endpoint and tests are unaffected.
...
- **Resilient startup migration** (`DatabaseInitializer`): tolerates concurrent "Database already exists" errors (rolling-deploy race where two pods migrate at once) and retries transient failures, instead of crash-looping. The `IsBenign` check is intentionally narrow (only matches `Database '...' already exists`) to avoid swallowing real migration failures like constraint-name conflicts. 5 tests.
...
| mypassport1000.CTF | 20.3 MB | 5 | ~306,000 | WD My Passport 1 TB (NTFS + exFAT + TrueCrypt) |
