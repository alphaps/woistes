...
- **Code coverage** (GitHub Actions, informational): a `coverage` job runs in parallel with `test`, collects coverage via `coverlet.collector` (`--collect:"XPlat Code Coverage"`), renders an HTML + Markdown report with ReportGenerator (posted to the job summary, uploaded as an artifact). It is `continue-on-error` and in no `needs` chain, so it never blocks build/deploy. Kept off the gating `test` job because instrumentation slows the parser tests (~12s → ~2m). Current line coverage ~92% overall, parser 100%.
